...
curl_fft_slope = pd.DataFrame(columns = np.arange(-24,1,1))#np.zeros(len(curl_data_init))
curl_fft_slope_merit = np.zeros(len(curl_data_init))
end_init = '2022-03-01'
end_final = '2022-03-10'
region_of_interest = curl_data_init.loc[end_init:end_final:3].index
filt_val = 15e-3
for idx_1, end_datetime in enumerate(region_of_interest):
    ...
    for idx_2, each in enumerate(start_offset_list):
        ...
        time_start = time_end - pd.DateOffset(years = 2) + pd.DateOffset(hours = int(each))
    ...
    curl_fft_slope.loc[end_datetime] = ((curl_data_filt_df/curl_data_filt_df.abs().max()).diff().iloc[-25:].values)
    #curl_fft_slope = curl_fft_slope.append((curl_data_filt_df/curl_data_filt_df.abs().max()).diff()[-24:])
    curl_fft_slope_merit[idx_1] = offset_merit_new
    print(idx_1, "/", len(region_of_interest), ":   ", offset_merit_new, ":   ", curl_fft_slope[-12][end_datetime])
...
curl_fft_slope_series[curl_fft_slope_series.abs() < 0.0015 ] = float('Nan')
